...
void showMeetingRecording(uint32_t elapsedSec) {
  clearWhite();
  // Big REC indicator — filled circle + label
  fillCircle(22, 22, 12, BLACK);
  drawStr(42, 14, "REC", 2, BLACK);

  // Elapsed time
  char buf[16];
  uint32_t m = elapsedSec / 60;
  uint32_t s = elapsedSec % 60;
  snprintf(buf, sizeof(buf), "%02lu:%02lu", (unsigned long)m, (unsigned long)s);
  drawBigStrC(100, 70, 48, buf, BLACK);

  hline(0, 130, W, BLACK);
  drawStrC(100, 148, "Press REC to stop", 1, BLACK);
  refresh();
}

void showMeetingDone() {
  clearWhite();
  iconCheck(100, 70, true);
