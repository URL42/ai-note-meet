...
// Full-refresh waveform LUT for 1.54" SSD1681 B&W panel
...
// Partial-refresh waveform LUT — fast flicker-free updates (~300ms)
...
void epaper_driver_display::spi_port_init()
{
    ...
    devcfg.clock_speed_hz = 40 * 1000 * 1000;  // SSD1681 supports 40 MHz (UC8151 was 4 MHz)
    devcfg.mode           = 0;
    devcfg.queue_size     = 7;
...
{
    int busy = lcd_spi_data.busy;
    // SSD1681: BUSY HIGH = busy, LOW = idle (opposite of UC8151)
...
void epaper_driver_display::EPD_Init()
{
    ...
    EPD_SendCommand(0x01);  // gate driver voltage
    EPD_SendData(0xC7);
    EPD_SendData(0x00);
    EPD_SendData(0x01);
    ...
    EPD_SendCommand(0x18);  // temperature sensor: internal
    EPD_SendData(0x80);
...
void epaper_driver_display::EPD_DisplayPartBaseImage()
{
    ...
    EPD_SendCommand(0x26);  // write "old" frame buffer — partial refresh diffs against this
    writeBytes(buffer, buffer_len);
...
{
    if (x >= (uint16_t)Width || y >= (uint16_t)Height) {
